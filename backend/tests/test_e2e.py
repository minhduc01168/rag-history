...
def test_embedding_service_health():
    ...
    try:
        response = requests.get(f"{EMBEDDING_URL}/health", timeout=3)
...
def test_embedding_generation():
    ...
    try:
        ...
        response = requests.post(f"{EMBEDDING_URL}/embed", json=payload, timeout=5)
        assert response.status_code == 200
        data = response.json()
...
def test_backend_rag_chat_e2e():
    """Kiểm tra luồng RAG Chat"""
    try:
        # Kiểm tra backend health trước
        h_res = requests.get("http://localhost:8000/health", timeout=3)
        if h_res.status_code != 200:
            pytest.skip("Backend is not healthy.")
        payload = {"query": "Vua Hùng dựng nước Văn Lang như thế nào?"}
        response = requests.post(f"{BACKEND_URL}/rag/chat", json=payload, timeout=10)
        assert response.status_code == 200
        data = response.json()
